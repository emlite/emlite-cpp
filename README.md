...
cp yourwasmfile.wasm dist/
```

Modify your package.json:
- Point the entry `main` to `./src/index.js`:
```json
"main": "src/index.js",
```
- Add webpack command to your `scripts` entry:
```json
  "scripts": {
    "build": "webpack",
...
In your index.html (note that main.js is generated by webpack in the dist folder):
```html
<!DOCTYPE html>
<html lang="en">
<head>
...
async function main() => {
    ...
    let wasi = new WASI([], [], fds);
    let emlite = new Emlite();
    let wasm = await WebAssembly.compileStreaming(fetch("./yourwasmfile.wasm"));
    let inst = await WebAssembly.instantiate(wasm, {
        "wasi_snapshot_preview1": wasi.wasiImport,
        "env": emlite.env,
    });
